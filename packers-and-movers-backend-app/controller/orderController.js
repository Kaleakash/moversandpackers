...
module.exports.findAllOrders = async function(req, res){
    ...
    if(result){
        return res.status(200).json({
            message:"All Orders records are",
            data:result
            });        
    }   
    }catch(err){
...
module.exports.findOrdertByEmailId = async function(req, res){
    let customerEmail = req.params.emailid;
    try{
        let result = await orderModel.find({email:customerEmail})
    if(result.length!=0){
        return res.status(200).json({
